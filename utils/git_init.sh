...
Використання:
  ...
  $SCRIPT_NAME <dir_name> <remote_url>    - ініціалізація + підключення віддаленого репозиторію
...
# скрипт запускається з директорії проєктів, а не зсередини репозиторію 
if [ -d "./.git" ]; then
...
# завантажити або запропонувати створити кофігурацію
...
# параметр: створити/ініціалізувати директорію-репозиторій 
if [ "$#" -eq 1 ]; then
    if [ -d "$DIR_NAME" ]; then
...
#  параметри: директорія + remote_url 
if [ "$#" -eq 2 ]; then
    if [ ! -d "$DIR_NAME" ]; then
        mkdir -p "$DIR_NAME"
        init_repo_in_dir "$DIR_NAME"
